...
│   │   ├── seed.py              # 演示数据生成与启动引导
│   │   ├── domain/              # 业务规则: 因子限值、枚举、超标分级、达标率口径
...
- **无 1 小时限值的因子**(PM2.5、PM10 小时值)仅记录数值, 不参与达标率分母和分子, 不能按达标处理。
- **标注状态**: `待标注(pending)` 由系统自动创建, 人工标注为 `已确认(confirmed)` 或 `已忽略(ignored)`; 确认与忽略都必须填写标注说明, 用于后续追溯。

### 达标率统一口径

达标率、有效超标数和参评记录数统一由后端 `app/domain/compliance.py`、`app/domain/compliance_sql.py` 与 `query_service` 的同一组查询条件计算, 明细列表、首页看板、聚合报表和 CSV 导出共用同一筛选结果, 不受分页页码、每页条数或导出上限影响。

| 数据状态 | 明细列表 | 看板 / 聚合报表 | CSV 导出 |
| --- | --- | --- | --- |
| 有限值、未超标 | 标记“达标” | 计入分子和分母 | 导出数据行, 计入统计页 |
| 有限值、超标且状态为待标注/已确认 | 标记“超标” | 计入有效超标和分母, 不计入达标分子 | 导出数据行, 计入统计页 |
| 超标记录已标记为 `ignored`(无效) | 保留并标记“无效(不参评)” | 不计入有效超标、达标数或达标率分母 | 保留原始数据和标注说明, 但统计页列为“已标记无效记录数(不参评)” |
| 因子在该周期没有限值 | 保留并标记“无限值(不参评)” | 不计入达标数、超标数或分母 | 保留原始数据, 统计页列为“无限值记录数(不参评)” |

计算公式:

```text
达标率 = 达标记录数 / (达标记录数 + 有效超标记录数)
有效超标记录数 = 自动判定超标且未标记为 ignored 的记录数
```

CSV 最多导出前 20000 行, 但文件末尾的统计页始终按全部筛选结果计算, 并单独写明本次实际导出行数。
...
```bash
cd backend
python -m pytest -q          # 47 个用例: 台账 CRUD/级联、录入与超标判定、标注规则、统一达标率、查询统计与导出、元数据接口
